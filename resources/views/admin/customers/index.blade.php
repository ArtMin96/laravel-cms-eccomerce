            ...
            <div class="page-header-content">
                <div class="row align-items-center justify-content-between pt-3">
                    ...
                    <div class="col-12 col-xl-auto mb-3">
                        <a href="{{ url('admin/customers/create') }}" class="btn btn-sm btn-primary" type="button">{{ __('Create customer') }}</a>
                    </div>
                </div>
            </div>
        </div>
                ...
                <div class="datatable">
                    <table class="table table-bordered table-hover" id="dataTable" width="100%" cellspacing="0">
                        ...
                        @foreach($customers as $customer)
                            <tr>
                                <td>
                                    <img src="{{ asset('storage/customers/'.$customer->image) }}" height="150" alt="Customer">
                                </td>
                                <td>@if(!empty($customer->deleted_at)) ? <div class="badge badge-danger badge-pill">{{ __('Inactive') }}</div> @else <div class="badge badge-success badge-pill">{{ __('Active') }}</div> @endif</td>
                                <td>{{ date('Y-m-d H:i', strtotime($customer->created_at)) }}</td>
                                <td>{{ date('Y-m-d H:i', strtotime($customer->updated_at)) }}</td>
                                <td>
                                    <a class="btn btn-datatable btn-icon btn-transparent-dark" href="{{ url('admin/customers/'.$customer->id.'/edit') }}">
                                        <i data-feather="edit-3"></i>
                                    </a>
                                    <button type="submit"
                                            class="btn btn-datatable btn-icon text-danger remove-page"
                                            data-page-id="{{ $customer->id }}"
                                            data-url="{{ url('/admin/customers/destroy') }}"
