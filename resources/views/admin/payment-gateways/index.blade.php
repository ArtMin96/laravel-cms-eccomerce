                    ...
                    <table class="table table-bordered table-hover" id="dataTable" width="100%" cellspacing="0">
                        ...
                        @foreach($paymentGateways as $gateway)
                            <tr>
                                <td>{{ $gateway->name }}</td>
                                <td>
                                    <img src="{{ asset($gateway->icon) }}" alt="{{ $gateway->name }}">
                                </td>
                                <td>@if(!empty($gateway->deleted_at)) ? <div class="badge badge-danger badge-pill">{{ __('Inactive') }}</div> @else <div class="badge badge-success badge-pill">{{ __('Active') }}</div> @endif</td>
                                <td>{{ date('Y-m-d H:i', strtotime($gateway->created_at)) }}</td>
                                <td>{{ date('Y-m-d H:i', strtotime($gateway->updated_at)) }}</td>
                                <td>
                                    <a class="btn btn-datatable btn-icon btn-transparent-dark" href="{{ url('admin/payment-gateways/'.$gateway->id.'/edit') }}">
