            ...
            <div class="page-header-content">
                <div class="row align-items-center justify-content-between pt-3">
                    ...
                    <div class="col-12 col-xl-auto mb-3">
                        <a href="{{ url('admin/faqs/create') }}" class="btn btn-sm btn-primary" type="button">{{ __('Create faq') }}</a>
                    </div>
                </div>
            </div>
        </div>
                        ...
                        <tbody>
                            @foreach($faqs as $faq)
                                <tr>
                                    ...
                                    <td>@if(!empty($faq->deleted_at)) ? <div class="badge badge-danger badge-pill">{{ __('Inactive') }}</div> @else <div class="badge badge-success badge-pill">{{ __('Active') }}</div> @endif</td>
                                    <td>{{ date('Y-m-d H:i', strtotime($faq->created_at)) }}</td>
                                    <td>{{ date('Y-m-d H:i', strtotime($faq->updated_at)) }}</td>
                                    <td>
                                        <a class="btn btn-datatable btn-icon btn-transparent-dark" href="{{ url('admin/faqs/'.$faq->id.'/edit') }}">
                                            <i data-feather="edit-3"></i>
                                        </a>
                                        <button type="submit"
                                                class="btn btn-datatable btn-icon text-danger remove-page"
                                                data-page-id="{{ $faq->id }}"
                                                data-url="{{ url('/admin/faqs/destroy') }}"
